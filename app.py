...
st.set_page_config(page_title="미국 주식 시그니처 터미널 v26.4.24.15", layout="wide", initial_sidebar_state="expanded")
...
if menu == "📍 시장 주요 지표":
    ...
    if target:
        ...
        if not hist.empty:
            ...
            m5.metric("52주 최고가", f"${info.get('fiftyTwoWeekHigh', 0):,.2f}"); m6.metric("52주 최저가", f"${info.get('fiftyTwoWeekLow', 0):,.2f}"); m7.metric("평균 거래량", f"{info.get('averageVolume', 0)/1e6:.1f}M"); m8.metric("배당률", div_fmt) 
...
st.sidebar.divider()
st.sidebar.markdown(f"<div style='text-align: center; color: #888; font-size: 0.95rem; font-weight: 600;'>v26.4.24.15 | {datetime.now().strftime('%H:%M:%S')}</div>", unsafe_allow_html=True)
