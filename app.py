...
if menu == "📍 시장 주요 지표":
    ...
    if target:
        ...
        if not hist.empty:
            ...
            m1.metric("시가총액", f"${info.get('marketCap', 0)/1e9:.1f}B"); m2.metric("현재 주가", f"${hist['Close'].iloc[-1]:,.2f}"); m3.metric("P/E Ratio", f"{info.get('trailingPE', 0):.2f}" if info.get('trailingPE') else "N/A"); m4.metric("EPS (TTM)", f"${info.get('trailingEps', 0):.2f}")
