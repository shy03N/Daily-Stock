...
# 페이지 설정
st.set_page_config(page_title="미국 주식 시그니처 터미널 v26.4.22.6", layout="wide", initial_sidebar_state="expanded")

# 2. 통합 CSS 스타일링 (마스터 백업본 100% 유지)
st.markdown("""
    <style>
    ...
    .detail-header-text { font-size: 1.2rem !important; font-weight: 700; color: #ffffff; margin-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 10px; }
...
def load_data():
    ...
    # 파일이 없으면 헤더만 있는 데이터프레임 생성
    return pd.DataFrame(columns=["Ticker", "Price", "Quantity"])

def save_data(df): df.to_csv(DB_FILE, index=False)

# 롤링 애니메이션 엔진 (사용자님이 만족하신 v26.4.22.3 핵심 기능 100% 유지)
...
if menu == "📍 시장 주요 지표":
    ...
    # 🔥 [v26.4.22.6] 통화 버튼 전용 격리 CSS 100% 보존
    st.markdown("""
        <style>
        ...
        [data-testid="column"]:nth-child(2) div[data-baseweb="select"] > div:first-child { padding-right: 0 !important; justify-content: center !important; }
    ...
    if 'show_manage_v84' not in st.session_state: st.session_state.show_manage_v84 = False
    ...
    # 🔥 [v26.4.22.6 핵심] 데이터가 있든 없든 기본 변수는 0으로 세팅하여 화면 유지
    ...
    ret_pct = round(((total_v - total_inv) / total_inv * 100) if total_inv > 0 else 0, 1)
    day_pct = round(((total_v - total_prev_day) / total_prev_day * 100) if total_prev_day > 0 else 0, 1)

    # 대시보드 카드 배치 (데이터가 없어도 나타남)
    ...
    # ⚙️ [v26.4.22.6 핵심] 종목이 없을 때도 추가 메뉴를 바로 볼 수 있게 상단으로 올림
    st.markdown('<div style="margin-top: 15px;"></div>', unsafe_allow_html=True)
    with st.expander("⚙️ 종목 관리 메뉴 (추가/삭제)", expanded=portfolio_df.empty):
        e1, e2, e3 = st.columns(3)
        t_in = e1.text_input("티커 (예: TSLA, NVDA)").upper()
        p_in = e2.number_input("평단가(USD)", 0.0)
        q_in = e3.number_input("보유수량", 0.0)
        if st.button("내 포트폴리오에 저장"):
            if t_in:
                new_data = pd.DataFrame([{'Ticker': t_in, 'Price': p_in, 'Quantity': q_in}])
                save_data(pd.concat([portfolio_df, new_data], ignore_index=True))
                st.rerun()
        if not portfolio_df.empty:
            del_target = st.selectbox("삭제할 종목 선택", portfolio_df['Ticker'].tolist())
            if st.button("선택 종목 삭제"):
                save_data(portfolio_df[portfolio_df['Ticker'] != del_target])
                st.rerun()

    # 상세 분석창 (데이터 있을 때만 상세 내용 표시)
    if st.session_state.show_portfolio_detail and not portfolio_df.empty:
    ...
    if not portfolio_df.empty:
        ...
        st.info("💡 종목 관리 메뉴에서 첫 종목을 추가하시면 자산 차트가 나타납니다.")

elif menu == "📊 종목 정밀 분석":
    st.markdown('<div class="main-title">📊 종목 정밀 분석</div>', unsafe_allow_html=True)
    portfolio = load_data()
    target = st.selectbox("분석할 종목 선택", portfolio['Ticker'].tolist() if not portfolio.empty else ["AAPL"], label_visibility="collapsed")
